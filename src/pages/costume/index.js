...
function CostumeIndex () {

    const [listsData, setData] = useState([]);

    const deleteItem = async(item) => {
        console.log('----------------- item: ', item);
        const user = doc(DBFirebase, 'dress', item.id)
        try{
    ...
    useEffect(() => {
        const q = query(collection(DBFirebase, 'dress'));
                ...
                <Row>
                    <Col>
                        <div className={'d-flex justify-content-between align-items-center'}>
                            <h1 className='heading-h1 mt-2 mb-2' >Thêm mới trang phục </h1>
                            <Link to={`/trang-phuc-them-moi`} className='nav-item'>Thêm mới</Link>
                        </div>
                    </Col>
                </Row>
                <Row>
